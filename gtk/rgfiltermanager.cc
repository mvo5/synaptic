     ...
     _selectedFilter(NULL), _filterview(filterview)
{
   ...
   gtk_cell_layout_add_attribute(
      GTK_CELL_LAYOUT(_comboPatternWhat), crt, "text", 0);
   ...
   gtk_cell_layout_add_attribute(
      GTK_CELL_LAYOUT(_comboPatternDo), crt, "text", 0);
   ...
   g_signal_connect(gtk_builder_get_object(_builder, "entry_pattern_text"),
                    ...
                    this);
   g_signal_connect(
      G_OBJECT(_comboPatternWhat), "changed", G_CALLBACK(patternChanged), this);
   g_signal_connect(
      G_OBJECT(_comboPatternDo), "changed", G_CALLBACK(patternChanged), this);
   ...
   g_signal_connect(
      gtk_builder_get_object(_builder, "button_status_select_all"),
      "clicked",
      G_CALLBACK(statusAllClicked),
      this);
   g_signal_connect(
      gtk_builder_get_object(_builder, "button_status_select_none"),
      "clicked",
   ...
   g_signal_connect(
      G_OBJECT(_win), "close-request", G_CALLBACK(deleteEventAction), this);
   ...
   column = gtk_tree_view_column_new_with_attributes(
      "Name", renderer, "text", NAME_COLUMN, NULL);
   ...
   g_signal_connect(
      G_OBJECT(select), "changed", G_CALLBACK(selectAction), this);
   ...
   renderer = gtk_cell_renderer_text_new();
   column = gtk_tree_view_column_new_with_attributes(
      "Name", renderer, "text", SECTION_COLUMN, NULL);
   ...
   for (set<string>::iterator I = sections.begin(); I != sections.end(); I++) {
      ...
      gtk_list_store_set(
         _sectionListStore, &iter, SECTION_COLUMN, (*I).c_str(), -1);
   }
   ...
   _patternListStore = gtk_list_store_new(
      PATTERN_N_COLUMNS, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING);
   renderer = gtk_cell_renderer_text_new();
   column = gtk_tree_view_column_new_with_attributes(
   ...
   gtk_tree_selection_set_mode(select, GTK_SELECTION_SINGLE);
   g_signal_connect(
      G_OBJECT(select), "changed", G_CALLBACK(patternSelectionChanged), this);
...
void RGFilterManagerWindow::filterNameChanged(GObject *o, gpointer data)
{
   ...
   if (gtk_tree_model_get_iter(
          GTK_TREE_MODEL(me->_filterListStore), &iter, me->_selectedPath)) {
...
void RGFilterManagerWindow::patternChanged(GObject *o, gpointer data)
{
   ...
   g_signal_handlers_block_by_func(
      G_OBJECT(w), (GClosure *)patternChanged, data);
...
gint RGFilterManagerWindow::deleteEventAction(GtkWidget *widget, gpointer data)
{
   RGFilterManagerWindow *me = (RGFilterManagerWindow *)data;
...
void RGFilterManagerWindow::readFilters()
{
   ...
   for (unsigned int i = 0; i < filters.size(); i++) {
      ...
      gtk_list_store_set(_filterListStore,
                         &iter,
                         NAME_COLUMN,
                                         ...
                                         gpointer data)
{
   ...
   if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
      ...
      gtk_tree_model_get(
         model, &iter, NAME_COLUMN, &filtername, FILTER_COLUMN, &filter, -1);
                                          ...
                                          string pattern)
{
   ...
   if (row < 0) {
      ...
      if (gtk_tree_model_get_iter(
             GTK_TREE_MODEL(_patternListStore), &iter, path)) {
         gtk_list_store_set(GTK_LIST_STORE(_patternListStore),
                            &iter,
                            0,
                            _(array[0]),
...
void RGFilterManagerWindow::getSectionFilter(RSectionPackageFilter &f)
{
   ...
   while (list) {
      if (gtk_tree_model_get_iter(GTK_TREE_MODEL(_sectionListStore),
         ...
         gtk_tree_model_get(GTK_TREE_MODEL(_sectionListStore),
                            &iter,
                            SECTION_COLUMN,
                            &text,
                            -1);
   ...
   while (valid) {
      ...
      for (size_t j = 0; DepOptions[j] != NULL; j++) {
         if (strcmp(what, _(DepOptions[j])) == 0) {
            ...
            RPatternPackageFilter::DepType type =
               (RPatternPackageFilter::DepType)j;
...
void RGFilterManagerWindow::addFilterAction(GtkWidget *self, void *data)
{
   ...
   gtk_list_store_set(me->_filterListStore,
                      &iter,
                      NAME_COLUMN,
                      filter->getName().c_str(),
                      FILTER_COLUMN,
...
void RGFilterManagerWindow::applyFilterAction(GtkWidget *self, void *data)
{
   ...
   gtk_tree_model_get(
      GTK_TREE_MODEL(me->_filterListStore), &iter, FILTER_COLUMN, &filter, -1);
   if (filter == NULL) {

...
void RGSummaryWindow::buildTree(RGSummaryWindow *me)
{
   ...
   if (notAuthenticated.size() > 0) {
      ...
      gtk_tree_store_set(
         me->_treeStore, &iter, PKG_COLUMN, _("NOT AUTHENTICATED"), -1);

      for (vector<string>::const_iterator p = notAuthenticated.begin();
           p != notAuthenticated.end();
           p++) {
         gtk_tree_store_append(me->_treeStore, &iter_child, &iter);
         gtk_tree_store_set(
            me->_treeStore, &iter_child, PKG_COLUMN, (*p).c_str(), -1);
      }
   }
#endif
   ...
   if (essential.size() > 0) {
      /* (Essentail) removed */
      gtk_tree_store_append(me->_treeStore, &iter, NULL);
      gtk_tree_store_set(me->_treeStore,
                         &iter,
                         PKG_COLUMN,
                         _("<b>(ESSENTIAL) to be removed</b>"),
                         -1);
      ...
      for (vector<RPackage *>::const_iterator p = essential.begin();
         ...
         gtk_tree_store_append(me->_treeStore, &iter_child, &iter);
         gtk_tree_store_set(
            me->_treeStore, &iter_child, PKG_COLUMN, (*p)->name(), -1);
      }
   }
   if (toDowngrade.size() > 0) {
      ...
      gtk_tree_store_append(me->_treeStore, &iter, NULL);
      gtk_tree_store_set(
         me->_treeStore, &iter, PKG_COLUMN, _("<b>To be DOWNGRADED</b>"), -1);
   ...
   if (toRemove.size() > 0) {
      ...
      gtk_tree_store_append(me->_treeStore, &iter, NULL);
      gtk_tree_store_set(
         me->_treeStore, &iter, PKG_COLUMN, _("<b>To be removed</b>"), -1);
   ...
   if (toUpgrade.size() > 0) {
      gtk_tree_store_append(me->_treeStore, &iter, NULL);
      gtk_tree_store_set(
         me->_treeStore, &iter, PKG_COLUMN, _("To be upgraded"), -1);
   ...
   if (toInstall.size() > 0) {
      gtk_tree_store_append(me->_treeStore, &iter, NULL);
      gtk_tree_store_set(
         me->_treeStore, &iter, PKG_COLUMN, _("To be installed"), -1);
   ...
   if (toReInstall.size() > 0) {
      gtk_tree_store_append(me->_treeStore, &iter, NULL);
      gtk_tree_store_set(
         me->_treeStore, &iter, PKG_COLUMN, _("To be re-installed"), -1);
...
void RGSummaryWindow::buildLabel(RGSummaryWindow *me)
{
   ...
   me->_lister->getDetailedSummary(held,
                                   kept,
                                   essential,
                                   toInstall,
                                   toReInstall,
   ...
   : RGGtkBuilderWindow(wwin, "summary")
{
   ...
   g_signal_connect(
      G_OBJECT(button), "clicked", G_CALLBACK(clickedDetails), this);
   ...
   if (sizeChange > 0) {
      g_string_append_printf(msg_space,
                             _("%s of extra space will be used"),
                             SizeToStr(sizeChange).c_str());
   } else if (sizeChange < 0) {
      g_string_append_printf(msg_space,
                             _("%s of extra space will be freed"),
                             SizeToStr(sizeChange).c_str());
      sizeChange = -sizeChange;
   }

   g_string_append_printf(
      msg_space, _("\n%s have to be downloaded"), SizeToStr(dlSize).c_str());

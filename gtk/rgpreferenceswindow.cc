...
const char *RGPreferencesWindow::column_names[] = {"status",
                                                   "supported",
                                                   "name",
                                                   "section",
                                                   "component",
...
const char *RGPreferencesWindow::removal_actions[] = {N_("Keep Configuration"),
                                                      N_("Completely"),
                                                      NULL};

const char *RGPreferencesWindow::update_ask[] = {N_("Always Ask"),
                                                 N_("Ignore"),
                                                 N_("Automatically"),
                                                 NULL};

const char *RGPreferencesWindow::upgrade_method[] = {N_("Always Ask"),
                                                     N_("Default Upgrade"),
                                                     N_("Smart Upgrade"),
                                                     NULL};
                                                     ...
                                                     void *data)
{
   ...
   gchar *defaultDistro =
      (gchar *)g_object_get_data(G_OBJECT(self), "defaultDistro");
   if (strcmp(defaultDistro, "distro") == 0) {
      gtk_widget_set_sensitive(GTK_WIDGET(me->_comboDefaultDistro), TRUE);
      gchar *s = gtk_combo_box_text_get_active_text(
         GTK_COMBO_BOX_TEXT(me->_comboDefaultDistro));
      if (s != NULL)
         me->_defaultDistro = s;
   } else {
...
void RGPreferencesWindow::saveGeneral()
{
   ...
   GtkWidget *notebook = GTK_WIDGET(
      gtk_builder_get_object(_mainWin->getGtkBuilder(), "notebook_pkginfo"));
   gtk_notebook_set_show_tabs(GTK_NOTEBOOK(notebook), newval);

   if (newval) {
      GtkWidget *widget = GTK_WIDGET(
         gtk_builder_get_object(_mainWin->getGtkBuilder(), "button_details"));
...
void RGPreferencesWindow::saveColumnsAndFonts()
{
   ...
   if (newval) {
      ...
      g_value_set_string(
         &value, _config->Find("Volatile::orginalFontName", "Sans 10").c_str());
      g_object_set_property(
         G_OBJECT(gtk_settings_get_default()), "gtk-font-name", &value);
   }
   g_value_unset(&value);
...
void RGPreferencesWindow::cbSaveAction(GtkWidget *self, void *data)
{
   RGPreferencesWindow *me = (RGPreferencesWindow *)data;
   start_task([me]() -> task<void> { co_await me->saveAction(); });
}


task<void> RGPreferencesWindow::saveAction()
{
   saveGeneral();
   saveColumnsAndFonts();
   saveColors();
...
void RGPreferencesWindow::cbDoneAction(GtkWidget *self, void *data)
{
   RGPreferencesWindow *me = (RGPreferencesWindow *)data;
   start_task([me]() -> task<void> { co_await me->doneAction(); });
}

task<void> RGPreferencesWindow::doneAction()
{
   co_await saveAction();
   if (distroChanged) {
      hide();
      _lister->unregisterObserver(_mainWin);
      _mainWin->setTreeLocked(TRUE);
      if (!_lister->openCache()) {
         co_await _mainWin->showErrors();
         exit(1);
      }
      _mainWin->setTreeLocked(FALSE);
      _lister->registerObserver(_mainWin);
      _mainWin->refreshTable();
   }
   closeAction(nullptr, this);
}
...
void RGPreferencesWindow::changeDefaultFontAction(GtkWidget *self, void *data)
{
   ...
   start_task([me]() -> task<void> {
      co_await me->changeFont("Synaptic::FontName", "sans 10");
   });
}
...
void RGPreferencesWindow::changeTerminalFontAction(GtkWidget *self, void *data)
{
   auto me = static_cast<RGPreferencesWindow *>(data);
   start_task([me]() -> task<void> {
      co_await me->changeFont("Synaptic::TerminalFontName", "monospace 10");
   });
}

task<void> RGPreferencesWindow::changeFont(const char *propName,
...
void RGPreferencesWindow::readGeneral()
{
   ...
   gtk_spin_button_set_value(
      GTK_SPIN_BUTTON(_maxUndoE),
      _config->FindI("Synaptic::undoStackSize", UndoStackSize));
...
void RGPreferencesWindow::readDistribution()
{
   ...
   g_signal_connect(
      G_OBJECT(now), "toggled", G_CALLBACK(cbRadioDistributionChanged), this);
   ...
   // clear the combo box
   GtkTreeModel *model =
      gtk_combo_box_get_model(GTK_COMBO_BOX(_comboDefaultDistro));
   ...
   // set up combo-box
   g_signal_connect(G_OBJECT(_comboDefaultDistro),
                    "changed",
                    G_CALLBACK(cbArchiveSelection),
                    this);
   ...
   GtkListStore *distroStore = GTK_LIST_STORE(
      gtk_combo_box_get_model(GTK_COMBO_BOX(_comboDefaultDistro)));
   ...
   gtk_cell_layout_add_attribute(
      GTK_CELL_LAYOUT(_comboDefaultDistro), crt, "text", 0);
...
void RGPreferencesWindow::readTreeViewValues()
{
   ...
   for (unsigned int i = 0; i < columns.size(); i++) {
      ...
      gtk_list_store_set(store,
                         ...
                         TREE_NAME_COLUMN,
                         columns[i].name,
                         TREE_VISIBLE_NAME_COLUMN,
                         _(columns[i].visible_name),
                         -1);
   }
...
void RGPreferencesWindow::cbMoveColumnUp(GtkWidget *self, void *data)
{
   ...
   next = iter;
   GtkTreePath *path =
      gtk_tree_model_get_path(GTK_TREE_MODEL(me->_listColumns), &iter);
...
void RGPreferencesWindow::cbToggleColumn(GtkWidget *self,
                                         char *path_string,
                                         void *data)
{
   ...
   gtk_tree_model_get(
      GTK_TREE_MODEL(model), &iter, TREE_CHECKBOX_COLUMN, &res, -1);
   gtk_list_store_set(
   ...
   : RGGtkBuilderWindow(win, "preferences"), distroChanged(false)
{
   ...
   // cache
   _cacheLeave =
      GTK_WIDGET(gtk_builder_get_object(_builder, "radio_cache_leave"));
   _cacheClean =
      GTK_WIDGET(gtk_builder_get_object(_builder, "radio_cache_del_after"));
   // history
   _delHistory =
      GTK_WIDGET(gtk_builder_get_object(_builder, "radio_delete_history"));
   _keepHistory =
      GTK_WIDGET(gtk_builder_get_object(_builder, "radio_keep_history"));
   _spinDelHistory =
      GTK_WIDGET(gtk_builder_get_object(_builder, "spin_del_history"));
   _cacheAutoClean =
   ...
   _maxUndoE =
      GTK_WIDGET(gtk_builder_get_object(_builder, "spinbutton_max_undos"));
   assert(_maxUndoE);

   _comboRemovalAction =
      GTK_WIDGET(gtk_builder_get_object(_builder, "combo_removal_action"));

   comboStore = GTK_LIST_STORE(
      gtk_combo_box_get_model(GTK_COMBO_BOX(_comboRemovalAction)));
   ...
   gtk_cell_layout_add_attribute(
      GTK_CELL_LAYOUT(_comboRemovalAction), crt, "text", 0);
   gtk_combo_box_set_active(GTK_COMBO_BOX(_comboRemovalAction), 0);
   assert(_comboRemovalAction);

   _comboUpdateAsk =
      GTK_WIDGET(gtk_builder_get_object(_builder, "combo_update_ask"));
   comboStore =
      GTK_LIST_STORE(gtk_combo_box_get_model(GTK_COMBO_BOX(_comboUpdateAsk)));
   ...
   gtk_cell_layout_add_attribute(
      GTK_CELL_LAYOUT(_comboUpdateAsk), crt, "text", 0);
   gtk_combo_box_set_active(GTK_COMBO_BOX(_comboUpdateAsk), 0);
   assert(_comboUpdateAsk);

   _comboUpgradeMethod =
      GTK_WIDGET(gtk_builder_get_object(_builder, "combo_upgrade_method"));
   comboStore = GTK_LIST_STORE(
      gtk_combo_box_get_model(GTK_COMBO_BOX(_comboUpgradeMethod)));
   ...
   gtk_cell_layout_add_attribute(
      GTK_CELL_LAYOUT(_comboUpgradeMethod), crt, "text", 0);
   ...
   gtk_widget_set_tooltip_text(
      GTK_WIDGET(gtk_builder_get_object(_builder, "radiobutton_distro")),
      _("Prefer package versions from the selected "
        ...
        "distribution."));
   gtk_widget_set_tooltip_text(
      GTK_WIDGET(gtk_builder_get_object(_builder, "radiobutton_now")),
   ...
   gtk_widget_set_tooltip_text(
      GTK_WIDGET(gtk_builder_get_object(_builder, "radiobutton_ignore")),
...
#ifdef HAVE_RPM
   GtkWidget *w =
      GTK_WIDGET(gtk_builder_get_object(_builder, "menuitem_purge"));
   gtk_widget_hide(w);
   int delAction = _config->FindI("Synaptic::delAction", PKG_DELETE);
   // purge not available
   ...
   g_signal_connect(gtk_builder_get_object(_builder, "apply"),
                    "clicked",
                    G_CALLBACK(cbSaveAction), this);
   g_signal_connect(gtk_builder_get_object(_builder, "ok"),
                    "clicked",
                    G_CALLBACK(cbDoneAction), this);
                                                      ...
                                                      void *data)
{
   ...
   start_task([me]() -> task<void> {
      ...
      GtkWidget *entry_user =
         GTK_WIDGET(gtk_builder_get_object(dia_xml, "entry_username"));
      GtkWidget *entry_pass =
         GTK_WIDGET(gtk_builder_get_object(dia_xml, "entry_password"));

                                      ...
                                      gpointer               data)
{
   ...
   gtk_tree_model_get(model, &iter, STATUS_COLUMN, &toggle_item,
   ...
   /* set new value */
   gtk_list_store_set(GTK_LIST_STORE(model), &iter, STATUS_COLUMN, toggle_item,
                      -1);
   ...
   : RGGtkBuilderWindow(parent, "repositories"), _dirty(false)
{
   ...
   // build gtktreeview
   _sourcesListStore =
      gtk_list_store_new(N_SOURCES_COLUMNS, G_TYPE_BOOLEAN, G_TYPE_STRING,
   ...
   column   = gtk_tree_view_column_new_with_attributes(
      _("Type"), renderer, "text", TYPE_COLUMN, "foreground-rgba",
   ...
   renderer = gtk_cell_renderer_text_new();
   column   = gtk_tree_view_column_new_with_attributes(
      _("Vendor"), renderer, "text", VENDOR_COLUMN, "foreground-rgba",
      DISABLED_COLOR_COLUMN, NULL);
   gtk_tree_view_append_column(GTK_TREE_VIEW(_sourcesListView), column);
#ifndef HAVE_RPM
   ...
   renderer = gtk_cell_renderer_text_new();
   column   = gtk_tree_view_column_new_with_attributes(
      _("URI"), renderer, "text", URI_COLUMN, "foreground-rgba",
      DISABLED_COLOR_COLUMN, NULL);
   gtk_tree_view_append_column(GTK_TREE_VIEW(_sourcesListView), column);
   ...
   renderer = gtk_cell_renderer_text_new();
   column   = gtk_tree_view_column_new_with_attributes(
      _("Distribution"), renderer, "text", DISTRIBUTION_COLUMN,
      "foreground-rgba", DISABLED_COLOR_COLUMN, NULL);
   gtk_tree_view_append_column(GTK_TREE_VIEW(_sourcesListView), column);

   // section(s)
   renderer = gtk_cell_renderer_text_new();
   column   = gtk_tree_view_column_new_with_attributes(
      _("Section(s)"), renderer, "text", SECTIONS_COLUMN, "foreground-rgba",
      DISABLED_COLOR_COLUMN, NULL);
   gtk_tree_view_append_column(GTK_TREE_VIEW(_sourcesListView), column);
   ...
   g_signal_connect(G_OBJECT(select), "changed", G_CALLBACK(SelectionChanged),
                    this);
   ...
   gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(_optType), renderer, "text",
                                  COL_NAME, NULL);
   ...
   gtk_list_store_set(_optTypeMenu, &item, COL_NAME, "rpm", COL_TYPE,
                      ITEM_TYPE_RPM, -1);

   gtk_list_store_append(_optTypeMenu, &item);
   gtk_list_store_set(_optTypeMenu, &item, COL_NAME, "rpm-src", COL_TYPE,
                      ITEM_TYPE_RPMSRC, -1);

   gtk_list_store_append(_optTypeMenu, &item);
   gtk_list_store_set(_optTypeMenu, &item, COL_NAME, "rpm-dir", COL_TYPE,
                      ITEM_TYPE_RPMDIR, -1);

   gtk_list_store_append(_optTypeMenu, &item);
   gtk_list_store_set(_optTypeMenu, &item, COL_NAME, "rpm-src-dir", COL_TYPE,
                      ITEM_TYPE_RPMSRCDIR, -1);

   gtk_list_store_append(_optTypeMenu, &item);
   gtk_list_store_set(_optTypeMenu, &item, COL_NAME, "repomd", COL_TYPE,
                      ITEM_TYPE_REPOMD, -1);

   gtk_list_store_append(_optTypeMenu, &item);
   gtk_list_store_set(_optTypeMenu, &item, COL_NAME, "repomd-src", COL_TYPE,
                      ITEM_TYPE_REPOMDSRC, -1);
#else
   gtk_list_store_append(_optTypeMenu, &item);
   gtk_list_store_set(_optTypeMenu, &item, COL_NAME, _("Binary (deb)"),
                      COL_TYPE, ITEM_TYPE_DEB, -1);

   gtk_list_store_append(_optTypeMenu, &item);
   gtk_list_store_set(_optTypeMenu, &item, COL_NAME, _("Source (deb-src)"),
                      COL_TYPE, ITEM_TYPE_DEBSRC, -1);
#endif
   ...
   gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(_optVendor), renderer, "text",
                                  0, NULL);
   ...
   assert(_entryURI);
   _entryDist =
      GTK_WIDGET(gtk_builder_get_object(_builder, "entry_distribution"));
   assert(_entryDist);
   ...
   g_signal_connect(GTK_WIDGET(gtk_builder_get_object(_builder, "button_ok")),
                    "clicked", G_CALLBACK(DoOK), this);

   g_signal_connect(
      GTK_WIDGET(gtk_builder_get_object(_builder, "button_cancel")), "clicked",
      G_CALLBACK(DoCancel), this);

   g_signal_connect(
      GTK_WIDGET(gtk_builder_get_object(_builder, "button_edit_vendors")),
      "clicked", G_CALLBACK(VendorsWindow), this);
...
bool RGRepositoryEditor::Run()
{
   ...
   for (SourcesListIter it = _lst.SourceRecords.begin();
      ...
      gtk_list_store_set(
         _sourcesListStore, &iter, STATUS_COLUMN,
         !((*it)->Type & SourcesList::Disabled), TYPE_COLUMN,
         utf8((*it)->GetType().c_str()), VENDOR_COLUMN,
         utf8((*it)->VendorID.c_str()), URI_COLUMN, utf8((*it)->URI.c_str()),
         DISTRIBUTION_COLUMN, utf8((*it)->Dist.c_str()), SECTIONS_COLUMN,
         utf8(Sections.c_str()), RECORD_COLUMN, (gpointer)(*it),
         DISABLED_COLOR_COLUMN,
         (*it)->Type & SourcesList::Disabled ? &_gray : NULL, -1);
   }
...
void RGRepositoryEditor::UpdateVendorMenu()
{
   ...
   for (VendorsListIter it = _lst.VendorRecords.begin();
      ...
      gtk_list_store_append(_optVendorMenu, &item);
      gtk_list_store_set(_optVendorMenu, &item, 0, (*it)->VendorID.c_str(), 1,
                         (*it)->VendorID.c_str(), -1);
   }
}
...
void RGRepositoryEditor::DoAdd(GtkWidget *, gpointer data)
{
   ...
   gtk_list_store_set(me->_sourcesListStore, &iter, STATUS_COLUMN, 1,
                      TYPE_COLUMN, "", VENDOR_COLUMN, "", URI_COLUMN, "",
                      DISTRIBUTION_COLUMN, "", SECTIONS_COLUMN, "",
                      RECORD_COLUMN, (gpointer)(rec), DISABLED_COLOR_COLUMN,
                      NULL, -1);
   ...
   gtk_tree_view_set_cursor(GTK_TREE_VIEW(me->_sourcesListView), path, NULL,
                            false);
   gtk_tree_view_scroll_to_cell(GTK_TREE_VIEW(me->_sourcesListView), path, NULL,
                                TRUE, 0.0, 0.0);
   gtk_tree_path_free(path);
...
void RGRepositoryEditor::doEdit()
{
   ...
   /* repaint screen */
   gtk_list_store_set(
      _sourcesListStore, _lastIter, STATUS_COLUMN,
      !(rec->Type & SourcesList::Disabled), TYPE_COLUMN,
      utf8(rec->GetType().c_str()), VENDOR_COLUMN, utf8(rec->VendorID.c_str()),
      URI_COLUMN, utf8(rec->URI.c_str()), DISTRIBUTION_COLUMN,
      utf8(rec->Dist.c_str()), SECTIONS_COLUMN, utf8(Sect.c_str()),
      RECORD_COLUMN, (gpointer)(rec), DISABLED_COLOR_COLUMN,
   ...
   if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
      ...
      for (unsigned int I = 0; I < rec->NumSections; I++) {
         ...
         gtk_editable_insert_text(GTK_EDITABLE(me->_entrySect),
                                  utf8(rec->Sections[I].c_str()), -1, &pos);
...
void RGRepositoryEditor::DoUpDown(GtkWidget *self, gpointer data)
{
   ...
   if (!gtk_tree_model_get_iter(GTK_TREE_MODEL(me->_sourcesListStore), &iter2,
                                path))
      return;

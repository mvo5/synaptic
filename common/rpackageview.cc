...
void RPackageViewFilter::makePresetFilters()
{
   ...
   {
      filter = new RFilter();
      filter->pattern.addPattern(RPatternPackageFilter::Name, "^task-.*",
                                 false);
...
#ifndef HAVE_RPM
   {
      filter         = new RFilter();
      filter->preset = true;
      filter->pattern.addPattern(RPatternPackageFilter::Depends, "^debconf",
                                 false);
   ...
   filter->pattern.addPattern(RPatternPackageFilter::Component, "restricted",
                              true);

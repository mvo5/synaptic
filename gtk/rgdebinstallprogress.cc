...
void RGDebInstallProgress::conffile(gchar *conffile, gchar *status)
{
   ...
   string diff;
   char   buf[512];
   char  *cmd = g_strdup_printf("/usr/bin/diff -u %s %s", orig_file.c_str(),
                                new_file.c_str());
   FILE  *f   = popen(cmd, "r");
...

{
   ...
   gtk_box_pack_start(
      ...
      TRUE, TRUE, 0);
   g_signal_connect(G_OBJECT(_term), "key-press-event",
   ...
   g_signal_connect(menuitem, "activate", (GCallback)cbMenuitemClicked,
                    (void *)EDIT_COPY);
   ...
   g_signal_connect(_term, "contents-changed", G_CALLBACK(content_changed),
                    this);

   g_signal_connect(gtk_builder_get_object(_builder, "button_cancel"),
                    "clicked", G_CALLBACK(cbCancel), this);
                                               ...
                                               gpointer     user_data)
{
   ...
   if (event->keyval == GDK_c && event->state & GDK_CONTROL_MASK) {
      ...
      GtkWidget *dia = gtk_message_dialog_new(
         GTK_WINDOW(me->_win), GTK_DIALOG_DESTROY_WITH_PARENT,
         GTK_MESSAGE_WARNING, GTK_BUTTONS_YES_NO, "%s", summary);
      gtk_message_dialog_format_secondary_text(GTK_MESSAGE_DIALOG(dia), "%s",
                                               msg);

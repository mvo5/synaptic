...
static GType gtk_tag_tree_get_column_type(GtkTreeModel *tree_model, gint index);
...
GType gtk_tag_tree_get_type(void)
{
   ...
   if (!pkg_tree_type) {
      ...
      static const GInterfaceInfo tree_model_info = {
         (GInterfaceInitFunc)gtk_tag_tree_tree_model_init, NULL, NULL};

      pkg_tree_type = g_type_register_static(
         G_TYPE_OBJECT, "GtkTagTree", &pkg_tree_info, (GTypeFlags)0);

      g_type_add_interface_static(
         pkg_tree_type, GTK_TYPE_TREE_MODEL, &tree_model_info);
   }
...
static GtkTreeModelFlags gtk_tag_tree_get_flags(GtkTreeModel *tree_model)
{
   ...
   //   RPackageLister::treeDisplayMode mode =
   //   GTK_TAG_TREE(tree_model)->_lister->getTreeDisplayMode();
...
#   ifdef DEBUG_TREE
      cout << "Node: " << get_path(cur) << ":"
           << (levl[depth - 1] - cur->size()) << endl;
#   endif
   }
...
#   ifdef DEBUG_TREE
   cout << "get_path(): " << iter->user_data << " : " << iter->user_data2
        << endl;
#   endif
...
#   ifdef DEBUG_TREE_FULL
   cout << "column: " << column << " name: " << name << endl
        << " name: " + name;
#   endif
...
#   ifdef DEBUG_TREE
   cout << "Node: " << cur->tag() << " child: " << child << " item: " << item
        << endl;
#   endif
...
#   endif
   ...
   if (cur->size() > 0) {
      ...
      cout << "Found a node without child nodes nor items! (it should not be "
              "part of the tree)"
           << endl;
      iter->user_data2 = GINT_TO_POINTER(-1);
      iter->user_data3 = GINT_TO_POINTER(-1);

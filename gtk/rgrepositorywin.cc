...
task<void> RGRepositoryEditor::selectionChanged(GtkTreeSelection *selection)
{
   ...
   if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
      ...
      gtk_editable_set_text(GTK_EDITABLE(_entryURI), utf8(rec->URI.c_str()));
      gtk_editable_set_text(GTK_EDITABLE(_entryDist), utf8(rec->Dist.c_str()));
      gtk_editable_set_text(GTK_EDITABLE(_entrySect), "");

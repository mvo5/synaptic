...
void RGTasksWin::cbButtonOkClicked(GtkWidget *self, void *data)
{
   ...
   do {
      gtk_tree_model_get(model, &iter, TASK_CHECKBOX_COLUMN, &marked,
                         TASK_SENSITIVE_COLUMN, &activatable, TASK_NAME_COLUMN,
                         &taskname, -1);
...
void RGTasksWin::cbButtonDetailsClicked(GtkWidget *self, void *data)
{
   ...
   if (!gtk_tree_selection_get_selected(
          selection, (GtkTreeModel **)(&me->_store), &iter)) {
      return;
   }
   gchar *str;
   gtk_tree_model_get(GTK_TREE_MODEL(me->_store), &iter, TASK_NAME_COLUMN, &str,
                      -1);

   // ask tasksel about the selected task
   string cmd = _config->Find("Synaptic::taskHelperProg", "/usr/bin/tasksel") +
                " --task-desc " + string(str);
   string taskDescr;
   char   buf[255];
   FILE  *f = popen(cmd.c_str(), "r");
   while (fgets(buf, 254, f) != NULL) {
                                       ...
                                       gpointer               user_data)
{
   ...
   gtk_tree_model_get(GTK_TREE_MODEL(model), &iter, TASK_CHECKBOX_COLUMN, &res,
                      -1);
   gtk_list_store_set(GTK_LIST_STORE(model), &iter, TASK_CHECKBOX_COLUMN, !res,
                      -1);
   ...
   if (selected > 0)
      gtk_widget_set_sensitive(
         GTK_WIDGET(gtk_builder_get_object(me->_builder, "button_ok")), true);
   else
      gtk_widget_set_sensitive(
         GTK_WIDGET(gtk_builder_get_object(me->_builder, "button_ok")), false);
}
...
RGTasksWin::RGTasksWin(RGWindow *parent) : RGGtkBuilderWindow(parent, "tasks")
{
   ...
   GtkListStore *store = _store =
      gtk_list_store_new(TASK_N_COLUMNS, G_TYPE_BOOLEAN, G_TYPE_BOOLEAN,
   ...
   while (fgets(buf, 254, f) != NULL) {
      ...
      gtk_list_store_set(store, &iter, TASK_CHECKBOX_COLUMN, installed,
                         TASK_SENSITIVE_COLUMN, !installed, TASK_NAME_COLUMN,
                         name, TASK_DESCR_COLUMN, strArray[1], -1);
      g_strfreev(strArray);
   }
   pclose(f);
   GtkWidget        *tree;
   ...
   tree = _taskView =
      GTK_WIDGET(gtk_builder_get_object(_builder, "treeview_tasks"));
   select = gtk_tree_view_get_selection(GTK_TREE_VIEW(tree));
   gtk_tree_selection_set_mode(select, GTK_SELECTION_SINGLE);
   g_signal_connect(G_OBJECT(select), "changed",
                    G_CALLBACK(selection_changed_callback), this);
   GtkCellRenderer   *renderer;
   GtkTreeViewColumn *column;
   renderer = gtk_cell_renderer_toggle_new();
   g_object_set(renderer, "activatable", TRUE, NULL);
   g_signal_connect(renderer, "toggled", (GCallback)cell_toggled_callback,
                    this);
   column = gtk_tree_view_column_new_with_attributes(
      "Install", renderer, "active", TASK_CHECKBOX_COLUMN, "activatable",
      TASK_SENSITIVE_COLUMN, NULL);
...
#endif
   column = gtk_tree_view_column_new_with_attributes(
      "Description", renderer, "text", TASK_DESCR_COLUMN, NULL);
   gtk_tree_view_append_column(GTK_TREE_VIEW(tree), column);
   ...
   g_signal_connect(gtk_builder_get_object(_builder, "button_cancel"),
                    "clicked", (GCallback)cbButtonCancelClicked, this);
   g_signal_connect(gtk_builder_get_object(_builder, "button_details"),
                    "clicked", (GCallback)cbButtonDetailsClicked, this);
};

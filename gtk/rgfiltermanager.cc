     ...
     _selectedFilter(NULL), _filterview(filterview)
{
   ...
   _comboPatternWhat =
      GTK_WIDGET(gtk_builder_get_object(_builder, "combobox_pattern_what"));
   comboStore =
      GTK_LIST_STORE(gtk_combo_box_get_model(GTK_COMBO_BOX(_comboPatternWhat)));
   ...
   gtk_cell_layout_add_attribute(GTK_CELL_LAYOUT(_comboPatternWhat), crt,
                                 "text", 0);
   gtk_combo_box_set_active(GTK_COMBO_BOX(_comboPatternWhat), 0);
   assert(_comboPatternWhat);

   _comboPatternDo =
      GTK_WIDGET(gtk_builder_get_object(_builder, "combobox_pattern_do"));
   comboStore =
      GTK_LIST_STORE(gtk_combo_box_get_model(GTK_COMBO_BOX(_comboPatternDo)));
   ...
   gtk_cell_layout_add_attribute(GTK_CELL_LAYOUT(_comboPatternDo), crt, "text",
                                 0);
   ...
   g_signal_connect(gtk_builder_get_object(_builder, "button_filters_add"),
                    "clicked", G_CALLBACK(addFilterAction), this);

   g_signal_connect(gtk_builder_get_object(_builder, "button_filters_remove"),
                    "clicked", G_CALLBACK(removeFilterAction), this);

   g_signal_connect(gtk_builder_get_object(_builder, "button_ok"), "clicked",
                    G_CALLBACK(okAction), this);

   g_signal_connect(gtk_builder_get_object(_builder, "button_cancel"),
                    "clicked", G_CALLBACK(cancelAction), this);

   g_signal_connect(gtk_builder_get_object(_builder, "entry_pattern_text"),
                    "changed", G_CALLBACK(patternChanged), this);
   g_signal_connect(G_OBJECT(_comboPatternWhat), "changed",
                    G_CALLBACK(patternChanged), this);
   g_signal_connect(G_OBJECT(_comboPatternDo), "changed",
                    G_CALLBACK(patternChanged), this);
   g_signal_connect(gtk_builder_get_object(_builder, "button_pattern_new"),
                    "clicked", G_CALLBACK(patternNew), this);
   g_signal_connect(gtk_builder_get_object(_builder, "button_pattern_delete"),
                    "clicked", G_CALLBACK(patternDelete), this);

   g_signal_connect(
      gtk_builder_get_object(_builder, "button_status_select_all"), "clicked",
      G_CALLBACK(statusAllClicked), this);
   g_signal_connect(
      gtk_builder_get_object(_builder, "button_status_select_none"), "clicked",
      G_CALLBACK(statusNoneClicked), this);
   g_signal_connect(gtk_builder_get_object(_builder, "button_status_invert"),
                    "clicked", G_CALLBACK(statusInvertClicked), this);

   g_signal_connect(gtk_builder_get_object(_builder, "entry_filters"),
                    "changed", G_CALLBACK(filterNameChanged), this);
   ...
   _filterDetailsBox =
      GTK_WIDGET(gtk_builder_get_object(_builder, "vbox_filter_details"));
   assert(_filterDetailsBox);

   // filter list view
   _filterEntry = GTK_WIDGET(gtk_builder_get_object(_builder, "entry_filters"));
   _filterList =
      GTK_WIDGET(gtk_builder_get_object(_builder, "treeview_filters"));
   _filterListStore =
      gtk_list_store_new(N_COLUMNS, G_TYPE_STRING, G_TYPE_POINTER);
   ...
   g_signal_connect(G_OBJECT(select), "changed", G_CALLBACK(selectAction),
                    this);

   // section list
   const set<string> &sections = _filterview->getSections();
   _sectionList =
      GTK_WIDGET(gtk_builder_get_object(_builder, "treeview_sections"));
   ...
   for (set<string>::iterator I = sections.begin(); I != sections.end(); I++) {
      ...
      gtk_list_store_set(_sectionListStore, &iter, SECTION_COLUMN, (*I).c_str(),
                         -1);
   }
   ...
   // pattern stuff
   _patternList =
      GTK_WIDGET(gtk_builder_get_object(_builder, "treeview_pattern"));
   _patternListStore = gtk_list_store_new(PATTERN_N_COLUMNS, G_TYPE_STRING,
                                          G_TYPE_STRING, G_TYPE_STRING);
   renderer          = gtk_cell_renderer_text_new();
   column            = gtk_tree_view_column_new_with_attributes(
      _("Field"), renderer, "text", PATTERN_WHAT_COLUMN, NULL);
   gtk_tree_view_append_column(GTK_TREE_VIEW(_patternList), column);

   column = gtk_tree_view_column_new_with_attributes(
      _("Operator"), renderer, "text", PATTERN_DO_COLUMN, NULL);
   gtk_tree_view_append_column(GTK_TREE_VIEW(_patternList), column);

   renderer = gtk_cell_renderer_text_new();
   // g_object_set(G_OBJECT(renderer), "editable", TRUE, NULL);
   column = gtk_tree_view_column_new_with_attributes(
      _("Pattern"), renderer, "text", PATTERN_TEXT_COLUMN, NULL);
   gtk_tree_view_append_column(GTK_TREE_VIEW(_patternList), column);
...
#ifndef HAVE_DEBTAGS
   GtkWidget *notebook =
      GTK_WIDGET(gtk_builder_get_object(_builder, "notebook_details"));
   assert(notebook);
   // if(first_run)
...
void RGFilterManagerWindow::patternChanged(GObject *o, gpointer data)
{
   ...
   GtkWidget *w =
      GTK_WIDGET(gtk_builder_get_object(me->_builder, "entry_pattern_text"));
   g_signal_handlers_block_by_func(G_OBJECT(w), (GClosure *)patternChanged,
                                   data);
   ...
   g_signal_handlers_unblock_by_func(G_OBJECT(w), (GClosure *)patternChanged,
                                     data);
}
                                                    ...
                                                    gpointer          data)
{
   ...
   if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
      gtk_widget_set_sensitive(
         GTK_WIDGET(gtk_builder_get_object(me->_builder, "hbox_pattern")),
         TRUE);
      gtk_tree_model_get(model, &iter, PATTERN_DO_COLUMN, &dopatt,
                         PATTERN_WHAT_COLUMN, &what, PATTERN_TEXT_COLUMN, &text,
                         -1);
      ...
      if (strcmp(dopatt, ActOptions[0]) == 0)
         ...
         exclude = true;
      gtk_combo_box_set_active(GTK_COMBO_BOX(me->_comboPatternDo),
                               exclude ? 1 : 0);
...
void RGFilterManagerWindow::readFilters()
{
   ...
   GtkTreeSelection *selection =
      gtk_tree_view_get_selection(GTK_TREE_VIEW(_filterList));
   ...
   for (unsigned int i = 0; i < filters.size(); i++) {
      ...
      gtk_list_store_set(_filterListStore, &iter, NAME_COLUMN,
                         filter->getName().c_str(), FILTER_COLUMN, filter, -1);
   }
                                         ...
                                         gpointer          data)
{
   ...
   if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
      ...
      gtk_tree_model_get(model, &iter, NAME_COLUMN, &filtername, FILTER_COLUMN,
                         &filter, -1);
      me->_selectedPath = gtk_tree_model_get_path(model, &iter);
      // cout << "path is " << gtk_tree_path_to_string(me->_selectedPath) <<
      // endl;
...
// mvo: helper function
GtkTreePath *RGFilterManagerWindow::treeview_find_path_from_text(
   GtkTreeModel *model,
   char         *text)
{
   GtkTreeIter  iter;
...
void RGFilterManagerWindow::setSectionFilter(RSectionPackageFilter &f)
{
   ...
   GtkWidget *_inclGB =
      GTK_WIDGET(gtk_builder_get_object(_builder, "radiobutton_incl"));
   assert(_inclGB);
   GtkWidget *_exclGB =
      GTK_WIDGET(gtk_builder_get_object(_builder, "radiobutton_excl"));
   assert(_exclGB);
                                          ...
                                          string pattern)
{
   ...
   if (row < 0) {
      ...
      gtk_list_store_set(GTK_LIST_STORE(_patternListStore), &iter, 0,
                         _(array[0]), 1, _(array[1]), 2, array[2], -1);
      GtkTreeSelection *select =
      ...
      if (gtk_tree_model_get_iter(GTK_TREE_MODEL(_patternListStore), &iter,
                                  path)) {
         gtk_list_store_set(GTK_LIST_STORE(_patternListStore), &iter, 0,
                            _(array[0]), 1, _(array[1]), 2, array[2], -1);
      }
...
void RGFilterManagerWindow::setPatternFilter(RPatternPackageFilter &f)
{
   ...
   if (f.getAndMode())
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(gtk_builder_get_object(
                                      _builder, "radiobutton_properties_and")),
                                   TRUE);
   else
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(gtk_builder_get_object(
                                      _builder, "radiobutton_properties_or")),
                                   TRUE);
}
...
void RGFilterManagerWindow::getSectionFilter(RSectionPackageFilter &f)
{
   ...
   while (list) {
      if (gtk_tree_model_get_iter(GTK_TREE_MODEL(_sectionListStore), &iter,
...
void RGFilterManagerWindow::getPatternFilter(RPatternPackageFilter &f)
{
   ...
   bool valid =
      gtk_tree_model_get_iter_first(GTK_TREE_MODEL(_patternListStore), &iter);
   while (valid) {
      /* Walk through the list, reading each row */
      gtk_tree_model_get(GTK_TREE_MODEL(_patternListStore), &iter,
                         PATTERN_DO_COLUMN, &dopatt, PATTERN_WHAT_COLUMN, &what,
      ...
      valid =
         gtk_tree_model_iter_next(GTK_TREE_MODEL(_patternListStore), &iter);
      g_free(dopatt);
      g_free(what);
      g_free(text);
   }
   ...
   f.setAndMode(gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(
      gtk_builder_get_object(_builder, "radiobutton_properties_and"))));
}
...
void RGFilterManagerWindow::addFilterAction(GtkWidget *self, void *data)
{
   ...
   gtk_list_store_set(me->_filterListStore, &iter, NAME_COLUMN,
                      filter->getName().c_str(), FILTER_COLUMN, filter, -1);
   GtkTreeSelection *selection;
...
void RGFilterManagerWindow::removeFilterAction(GtkWidget *self, void *data)
{
   ...
   gtk_tree_model_get_iter(GTK_TREE_MODEL(me->_filterListStore), &iter,
                           me->_selectedPath);
   gtk_tree_model_get(GTK_TREE_MODEL(me->_filterListStore), &iter,
                      FILTER_COLUMN, &filter, -1);
   if (filter) {

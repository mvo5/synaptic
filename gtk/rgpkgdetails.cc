...
{
   g_signal_connect(gtk_builder_get_object(_builder, "button_close"), "clicked",
   ...
   g_signal_connect(G_OBJECT(label), "activate-link", G_CALLBACK(cbOpenLink),
                    NULL);
}
...
void RGPkgDetailsWindow::cbShowScreenshot(GtkWidget *button, void *data)
{
   ...
   if (_config->FindB("Synaptic::InlineScreenshots") == false) {
      ...
      g_signal_connect(G_OBJECT(event), "button_press_event",
                       G_CALLBACK(cbShowBigScreenshot), (void *)si->pkg);
                                      ...
                                      bool                setTitle)
{
   ...
   if (gtk_text_tag_table_lookup(tag_table, "bold") == NULL) {
      gtk_text_buffer_create_tag(buf, "bold", "weight", PANGO_WEIGHT_BOLD,
                                 "scale", 1.1, NULL);
   }
   // set summary
   s = utf8(pkg->summary());
   ...
   si.pkg      = pkg;
   g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(cbShowScreenshot),
                    &si);
   ...
   g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(cbShowChangelog),
                    pkg);
   gtk_text_view_add_child_at_anchor(GTK_TEXT_VIEW(textview), button, anchor);
   gtk_widget_show(button);
   ...
   if (strlen(pkg->homepage())) {
      ...
      anchor = gtk_text_buffer_create_child_anchor(buf, &it);
      button =
         gtk_link_button_new_with_label(pkg->homepage(), _("Visit Homepage"));
      char *homepage_tooltip = g_strdup_printf(_("Visit %s"), pkg->homepage());
      g_signal_connect(G_OBJECT(button), "activate-link",
                       G_CALLBACK(cbOpenHomepage), pkg);
      gtk_widget_set_tooltip_text(button, homepage_tooltip);
      g_free(homepage_tooltip);
      g_object_set_data(G_OBJECT(button), "me", me);
      gtk_text_view_add_child_at_anchor(GTK_TEXT_VIEW(textview), button,
                                        anchor);
      gtk_widget_show(button);
   }
   ...
   for (int i = 0; i < versions.size(); i++) {
      ...
      // e.g. "0.56 (unstable)"
      //      "0.53.4 (testing)"
      str = g_strdup_printf(_("%s (%s)"), versions[i].first.c_str(),

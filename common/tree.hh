/* 
   ...
   $Id: tree.hh,v 1.95 2003/08/01 15:56:49 kp229 Exp $
...
typename tree<T, tree_node_allocator>::sibling_iterator tree<T, tree_node_allocator>::next_sibling(const iterator_base& position) const
	{
	assert(position.node!=0);
	if(position.node->next_sibling==0 || position.node->next_sibling==feet) 
...
bool tree<T, tree_node_allocator>::equal_subtree(const iter& one_, const iter& two_) const
	{
	std::equal_to<T> comp;
	return equal_subtree(one_, two, comp);
	}
...
typename tree<T, tree_node_allocator>::fixed_depth_iterator& tree<T, tree_node_allocator>::fixed_depth_iterator::operator++()
	...
	if(node->next_sibling!=0) {
		node=node->next_sibling;
		assert(node!=0);
		if(node->parent==0 && node->next_sibling==0) // feet element
			node=0;
		}
	else {
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::fixed_depth_iterator& tree<T, tree_node_allocator>::fixed_depth_iterator::operator-=(unsigned int)
	{
	while(num>0) {
		--(*this);
		--num;

...
const char *RGPreferencesWindow::update_ask[] = {N_("Always Ask"), N_("Ignore"),
                                                 N_("Automatically"), NULL};
...
void RGPreferencesWindow::applyProxySettings()
{
   ...
   if (useProxy) {
      ...
      if (!httpUser.empty() && !httpPass.empty()) {
         s = g_strdup_printf(
            "http://%s:%s@%s:%i/", QuoteString(httpUser, "@%:/").c_str(),
            QuoteString(httpPass, "@%:/").c_str(), http.c_str(), httpPort);
         _config->Set("Acquire::http::Proxy", s);
         g_free(s);
      }
      if (!ftp.empty()) {
...
void RGPreferencesWindow::saveColumnsAndFonts()
{
   ...
   do {
      gtk_tree_model_get(GTK_TREE_MODEL(store), &iter, TREE_CHECKBOX_COLUMN,
                         &visible, TREE_NAME_COLUMN, &column_name, -1);
...
void RGPreferencesWindow::readColors()
{
   ...
   for (int i = 0; i < RGPackageStatus::N_STATUS_COUNT; i++) {
      color_button = g_strdup_printf(
         "button_%s_color", RGPackageStatus::pkgStatus.getShortStatusString(
...
void RGPreferencesWindow::readDistribution()
{
   ...
   g_signal_connect(G_OBJECT(ignore), "toggled",
   ...
   g_signal_connect(G_OBJECT(now), "toggled",
                    G_CALLBACK(cbRadioDistributionChanged), this);
   ...
   g_signal_connect(G_OBJECT(distro), "toggled",
                    G_CALLBACK(cbRadioDistributionChanged), this);
   ...
   g_signal_connect(gtk_builder_get_object(_builder, "entry_http_proxy"),
                    "changed", G_CALLBACK(cbHttpProxyEntryChanged), this);
   ...
   gtk_cell_layout_add_attribute(GTK_CELL_LAYOUT(_comboDefaultDistro), crt,
                                 "text", 0);
...
void RGPreferencesWindow::readTreeViewValues()
{
   ...
   for (unsigned int i = 0; i < columns.size(); i++) {
      ...
      gtk_list_store_set(store, &iter, TREE_CHECKBOX_COLUMN, columns[i].visible,
                                         ...
                                         void      *data)
{
   ...
   gtk_tree_model_get(GTK_TREE_MODEL(model), &iter, TREE_CHECKBOX_COLUMN, &res,
                      -1);
   gtk_list_store_set(GTK_LIST_STORE(model), &iter, TREE_CHECKBOX_COLUMN, !res,
                      -1);
}
   ...
   : RGGtkBuilderWindow(win, "preferences"), distroChanged(false)
{
   ...
   gtk_cell_layout_add_attribute(GTK_CELL_LAYOUT(_comboRemovalAction), crt,
                                 "text", 0);
   ...
   gtk_cell_layout_add_attribute(GTK_CELL_LAYOUT(_comboUpdateAsk), crt, "text",
                                 0);
   ...
   gtk_cell_layout_add_attribute(GTK_CELL_LAYOUT(_comboUpgradeMethod), crt,
                                 "text", 0);
   ...
   for (int i = 0; i < RGPackageStatus::N_STATUS_COUNT; i++) {
      ...
      g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(colorClicked),
                       GINT_TO_POINTER(i));
      g_free(color_button);
   }

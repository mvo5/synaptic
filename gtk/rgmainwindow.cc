...
const char *relOptions[] = {N_("Dependencies"), N_("Dependants"),
                            N_("Dependencies of the Latest Version"),
                            N_("Provided Packages"), NULL};
                                        ...
                                        gpointer       data)
{
   ...
   for (unsigned int i = 0; i < versions.size(); i++) {
      gchar      *str    = g_strdup_printf("%s (%s)", versions[i].first.c_str(),
                                  ...
                                  const vector<RPackage *> &exclude)
{
   ...
   if (ask && _lister->getStateChanges(state, toKeep, toInstall, toReInstall,
      ...
      changes.confirm(_lister, toKeep, toInstall, toReInstall, toUpgrade,
                      toRemove, toDowngrade, notAuthenticated);
                              ...
                              GdkModifierType mods)
{
   ...
   gtk_accel_group_connect(accel_group, key, mods, GTK_ACCEL_VISIBLE,
                           g_cclosure_new_swap(G_CALLBACK(acceleratorCallback),
                                               closure,
     ...
     _logView(0), _installProgress(0), _fetchProgress(0), _fastSearchEventID(-1)
{
   ...
   gtk_widget_insert_action_group(GTK_WIDGET(_win), "win",
   ...
   setActionShortcut(this, accel_group, "mark-all-upgrades", GDK_KEY_G,
                     GDK_CONTROL_MASK);
   ...
   setActionShortcut(this, accel_group, "mark-install", GDK_KEY_I,
                     GDK_CONTROL_MASK);
   setActionShortcut(this, accel_group, "mark-upgrade", GDK_KEY_U,
                     GDK_CONTROL_MASK);
   setActionShortcut(this, accel_group, "mark-delete", GDK_KEY_Delete,
                     (GdkModifierType)0);
   setActionShortcut(this, accel_group, "mark-purge", GDK_KEY_Delete,
                     GDK_SHIFT_MASK);
   setActionShortcut(this, accel_group, "override-version", GDK_KEY_E,
                     GDK_CONTROL_MASK);
   setActionShortcut(this, accel_group, "download-changelog", GDK_KEY_L,
                     GDK_CONTROL_MASK);
   setActionShortcut(this, accel_group, "package-properties", GDK_KEY_Return,
                     GDK_MOD1_MASK);
...
gboolean RGMainWindow::xapianDoIndexUpdate(void *data)
{
   ...
   if (g_spawn_async(NULL, const_cast<char **>(argp), NULL,
                     (GSpawnFlags)(G_SPAWN_DO_NOT_REAP_CHILD), NULL, NULL, &pid,
                     NULL)) {
...
void RGMainWindow::buildInterface()
{
   ...
   gtk_window_move(GTK_WINDOW(_win), _config->FindI("Synaptic::windowX", 100),
   ...
   gtk_menu_shell_bind_model(
      ...
      G_MENU_MODEL(gtk_builder_get_object(_builder, "main_menu")), nullptr,
      false);

   g_signal_connect(gtk_builder_get_object(_builder, "entry_fast_search"),
                    "changed", G_CALLBACK(cbSearchEntryChanged), this);
   ...
   g_signal_connect(G_OBJECT(comboDepends), "changed",
   ...
   gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(comboDepends), relRenderText,
                              FALSE);
   gtk_cell_layout_add_attribute(GTK_CELL_LAYOUT(comboDepends), relRenderText,
                                 "text", 0);
   ...
   g_signal_connect(G_OBJECT(select), "changed", G_CALLBACK(cbSelectedRow),
                    this);
   ...
   g_signal_connect(G_OBJECT(select), "changed", G_CALLBACK(cbChangedSubView),
                    this);
...
void RGMainWindow::saveState()
{
   ...
   if (gdk_window_get_state(gtk_widget_get_window(_win)) &
                                            ...
                                            gpointer        data)
{
   ...
   if (event->type == GDK_BUTTON_PRESS) {
      ...
      if (gtk_tree_view_get_path_at_pos(GTK_TREE_VIEW(treeview), (int)event->x,
                                        (int)event->y, &path, &column, NULL,
                                        NULL)) {
                                             ...
                                             gpointer           data)
{
   ...
   if (ok && gtk_tree_model_get_iter_first(me->_pkgList, &iter)) {
      gtk_tree_view_scroll_to_cell(GTK_TREE_VIEW(treeview), start, NULL, true,
                                   0.0, 0.0);
   }
   gtk_tree_path_free(start);
                                 ...
                                 gpointer       data)
{
   ...
   filesel = gtk_file_chooser_dialog_new(
      _("Open changes"), GTK_WINDOW(me->window()), GTK_FILE_CHOOSER_ACTION_OPEN,
                                   ...
                                   gpointer       data)
{
   ...
   GtkWidget *filesel;
   filesel = gtk_file_chooser_dialog_new(
      _("Save changes"), GTK_WINDOW(me->window()), GTK_FILE_CHOOSER_ACTION_SAVE,
                                       ...
                                       gpointer       data)
{
   ...
   if (Changed == true && ForceReload) {
      ...
      dialog = gtk_message_dialog_new(
         GTK_WINDOW(me->window()), GTK_DIALOG_DESTROY_WITH_PARENT,
         GTK_MESSAGE_INFO, GTK_BUTTONS_NONE, _("Repositories changed"));
      ...
      gtk_box_pack_start(
         GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(dialog))), cb, true,
         true, 0);
      gtk_widget_show(cb);
...
void RGMainWindow::cbShowAboutPanel(GSimpleAction *action,
                                    ...
                                    gpointer       data)
{
   const char *authors[]     = {"Alfredo K. Kojima",   "Michael Vogt",
                                "Gustavo Niemeyer",    "Sebastian Heinlein",
                                "Enrico Zini",         "Panu Matilainen",
                                "Sviatoslav Sviridov", NULL};
   const char *documenters[] = {"Wybo Dekker", "Michael Vogt",
                                "Sebastian Heinlein", NULL};

   gtk_show_about_dialog(
      NULL, "program-name", _("Synaptic Package Manager"), "version", VERSION,
      "logo-icon-name", "synaptic", "copyright",
      _("© 2001-2004 Connectiva S/A \n © 2002-2025 Michael Vogt"), "authors",
      authors, "documenters", documenters, "translator-credits",
      _("translator-credits"), "comments",
                                           ...
                                           gpointer       data)
{
   ...
   const gchar *cmd[] = {"/usr/sbin/dpkg-reconfigure", "-fgnome",
                         me->selectedPackage()->name(), NULL};
   GError      *error = NULL;
   g_spawn_async("/", const_cast<gchar **>(cmd), NULL, (GSpawnFlags)0, NULL,
                 NULL, NULL, &error);
   if (error != NULL) {
                                    ...
                                    gpointer       data)
{
   ...
   if (is_binary_in_path("dwww")) {
      ...
      cmd[2] = NULL;
      g_spawn_async("/tmp", const_cast<gchar **>(cmd), NULL, (GSpawnFlags)0,
                    NULL, NULL, NULL, NULL);
   } else {
                                       ...
                                       vector<RPackage *> selected_pkgs)
{
   ...
   g_menu_append(popupMenuModel, _("Mark for Installation"),
                 "win.mark-install");
   g_menu_append(popupMenuModel, _("Mark for Reinstallation"),
                 "win.mark-reinstall");
...
#ifndef HAVE_RPM
   g_menu_append(popupMenuModel, _("Mark for Complete Removal"),
                 "win.mark-purge");
#endif
   ...
   if (selected_pkgs.size() == 1) {
      ...
      if (GMenu *recommendedSubmenu =
             me->buildWeakDependsMenu(pkg, pkgCache::Dep::Recommends))
         ...
         g_menu_append(recommendsSection,
                       _("Mark Recommended for Installation"), nullptr);

      if (GMenu *suggestedSubmenu =
             me->buildWeakDependsMenu(pkg, pkgCache::Dep::Suggests))
         g_menu_append_submenu(recommendsSection,
                               _("Mark Suggested for Installation"),
                               G_MENU_MODEL(suggestedSubmenu));
      else
         g_menu_append(recommendsSection, _("Mark Suggested for Installation"),
                       nullptr);

      g_menu_append_section(popupMenuModel, nullptr,
                            G_MENU_MODEL(recommendsSection));
   }
#endif

   GdkRectangle rect{0, 0, 0, 0};
   gtk_tree_view_convert_bin_window_to_widget_coords(
      GTK_TREE_VIEW(treeview), (int)event->x, (int)event->y, &rect.x, &rect.y);

   GtkWidget *popupMenu =
      gtk_popover_new_from_model(treeview, G_MENU_MODEL(popupMenuModel));
      ...
      if (deps[i].type == type) {
         ...
         if (!deps[i].isVirtual) {
            ...
            if (!deps[i].isSatisfied)
               g_menu_item_set_action_and_target(item, "win.install-by-name",
                                                 "s", deps[i].name);
                                                   ...
                                                   gpointer       data)
{
   ...
   GtkWidget *filesel;
   filesel = gtk_file_chooser_dialog_new(
      _("Save script"), GTK_WINDOW(me->window()), GTK_FILE_CHOOSER_ACTION_SAVE,
      _("_Cancel"), GTK_RESPONSE_CANCEL, _("_Save"), GTK_RESPONSE_ACCEPT, NULL);
                                               ...
                                               gpointer       data)
{
   ...
   GtkWidget *filesel;
   filesel = gtk_file_chooser_dialog_new(
      _("Select directory"), GTK_WINDOW(me->window()),

...
void RGSummaryWindow::buildTree(RGSummaryWindow *me)
{
   ...
   if (notAuthenticated.size() > 0) {
      GtkWidget *label;
      label =
         GTK_WIDGET(gtk_builder_get_object(me->_builder, "label_auth_warning"));
      assert(label);
      ...
      gtk_tree_store_set(me->_treeStore, &iter, PKG_COLUMN,
                         _("NOT AUTHENTICATED"), -1);
      ...
      for (vector<string>::const_iterator p = notAuthenticated.begin();
         ...
         gtk_tree_store_set(me->_treeStore, &iter_child, PKG_COLUMN,
                            (*p).c_str(), -1);
      }
   }
#endif
   ...
   if (essential.size() > 0) {
      ...
      gtk_tree_store_append(me->_treeStore, &iter, NULL);
      gtk_tree_store_set(me->_treeStore, &iter, PKG_COLUMN,
                         _("<b>(ESSENTIAL) to be removed</b>"), -1);
   ...
   if (toPurge.size() > 0) {
      ...
      gtk_tree_store_append(me->_treeStore, &iter, NULL);
      gtk_tree_store_set(
         me->_treeStore, &iter, PKG_COLUMN,
         _("<b>To be completely removed (including configuration files)</b>"),
         -1);
   ...
   if (toUpgrade.size() > 0) {
      gtk_tree_store_append(me->_treeStore, &iter, NULL);
      gtk_tree_store_set(me->_treeStore, &iter, PKG_COLUMN, _("To be upgraded"),
                         -1);
   ...
   if (held.size() > 0) {
      gtk_tree_store_append(me->_treeStore, &iter, NULL);
      gtk_tree_store_set(me->_treeStore, &iter, PKG_COLUMN, _("Unchanged"), -1);
      for (vector<RPackage *>::const_iterator p = held.begin(); p != held.end();
           p++) {
         gtk_tree_store_append(me->_treeStore, &iter_child, &iter);
         gtk_tree_store_set(me->_treeStore, &iter_child, PKG_COLUMN,
                            (*p)->name(), -1);
...
void RGSummaryWindow::buildLabel(RGSummaryWindow *me)
{
   ...
   me->_lister->getDetailedSummary(held, kept, essential, toInstall,
                                   toReInstall, toUpgrade, toRemove, toPurge,
                                   toDowngrade,
#ifdef WITH_APT_AUTH
                                   notAuthenticated,
#endif
   ...
   for (vector<RPackage *>::const_iterator p = toUpgrade.begin();
        p != toUpgrade.end(); p++) {
      str = g_strdup_printf(_("<b>%s</b> (version <i>%s</i>) will be upgraded "
                              "to version <i>%s</i>\n"),
   ...
   for (vector<RPackage *>::const_iterator p = toInstall.begin();
        p != toInstall.end(); p++) {
      str =
         g_strdup_printf(_("<b>%s</b> (version <i>%s</i>) will be installed\n"),
   ...
   for (vector<RPackage *>::const_iterator p = toReInstall.begin();
        p != toReInstall.end(); p++) {
      str = g_strdup_printf(
         _("<b>%s</b> (version <i>%s</i>) will be re-installed\n"),
         (*p)->name(), (*p)->installedVersion());
      text += str;
      g_free(str);
   }
...
void RGSummaryWindow::clickedDetails(GtkWidget *self, void *data)
{
   ...
   s = GTK_WIDGET(
      gtk_builder_get_object(me->_builder, "scrolledwindow_summary"));
   d = GTK_WIDGET(
      gtk_builder_get_object(me->_builder, "scrolledwindow_details"));
   ...
   : RGGtkBuilderWindow(wwin, "summary")
{
   ...
   _summarySpaceL =
      GTK_WIDGET(gtk_builder_get_object(_builder, "label_summary_space"));
   ...
   // gtk_tree_view_column_new_with_attributes("Summary",renderer, PKG_COLUMN,
   // "pkg", NULL);
   GtkTreeViewColumn *column;
   column = gtk_tree_view_column_new_with_attributes(
      _("Marked Changes"), renderer, "markup", PKG_COLUMN, NULL);
   ...
   _dlonlyB =
      GTK_WIDGET(gtk_builder_get_object(_builder, "checkbutton_download_only"));

   _checkSigsB = GTK_WIDGET(
      gtk_builder_get_object(_builder, "checkbutton_check_signatures"));
   assert(_checkSigsB);
#ifdef HAVE_RPM
   ...
   button =
      GTK_WIDGET(gtk_builder_get_object(_builder, "togglebutton_details"));
   g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(clickedDetails),
                    this);
   ...
   lister->getSummary(held, kept, essential, toInstall, toReInstall, toUpgrade,
                      toRemove, toDowngrade, unAuthenticated, sizeChange);
   ...
   if (held) {
      char *str =
         ngettext("%d package will be held back and not upgraded\n",
   ...
   if (toReInstall) {
      char *str =
         ngettext("%d new package will be re-installed\n",
   ...
   if (toDowngrade) {
      char *str =
         ngettext("%d package will be <b>downgraded</b>\n",

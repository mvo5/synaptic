...
void RGTasksWin::cbButtonOkClicked(GtkWidget *self, void *data)
{
   ...
   if (act) {
      ...
      while (fgets(buf, 254, f) != NULL) {
         me->packages.push_back(string(g_strstrip(buf)));
      }
      pclose(f);
...
void RGTasksWin::cbButtonDetailsClicked(GtkWidget *self, void *data)
{
   ...
   start_task([me]() -> task<void> {
      ...
      gchar *str;
      gtk_tree_model_get(
         GTK_TREE_MODEL(me->_store), &iter, TASK_NAME_COLUMN, &str, -1);
      ...
      // TRANSLATORS: Title of the task window - %s is the task (e.g. "desktop"
      // or "mail server")
                                       ...
                                       gpointer user_data)
{
   ...
   gtk_tree_model_get(
      GTK_TREE_MODEL(model), &iter, TASK_CHECKBOX_COLUMN, &res, -1);
   gtk_list_store_set(
      GTK_LIST_STORE(model), &iter, TASK_CHECKBOX_COLUMN, !res, -1);
...
RGTasksWin::RGTasksWin(RGWindow *parent) : RGGtkBuilderWindow(parent, "tasks")
{
   ...
   g_signal_connect(
      renderer, "toggled", (GCallback)cell_toggled_callback, this);
   ...
   g_signal_connect(gtk_builder_get_object(_builder, "button_ok"),
                    "clicked",
                    (GCallback)cbButtonOkClicked,
                    this);
   g_signal_connect(gtk_builder_get_object(_builder, "button_cancel"),
                    "clicked",
                    (GCallback)cbButtonCancelClicked,
                    this);
   g_signal_connect(gtk_builder_get_object(_builder, "button_details"),
                    "clicked",
                    (GCallback)cbButtonDetailsClicked,
                    this);
};

task<std::vector<std::string>> RGTasksWin::selectTasks()
{
   packages.clear();
   co_await co_run_window(GTK_WINDOW(window()));

   co_return std::move(packages);
}

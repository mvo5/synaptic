...
void RGLogView::readLogs()
{
   ...
   GtkTreeStore *store = gtk_tree_store_new(N_LOG_COLUMNS, G_TYPE_STRING,
   ...
   while ((logfile = g_dir_read_name(dir)) != NULL) {
      if (sscanf(logfile, "%4u-%2u-%2u.%2u%2u%2u.log", &year, &month, &day,
                 &hour, &min, &sec) != 6)
         continue;
      ...
      if (history_map.count(history_key) == 0) {
         ...
         gtk_tree_store_set(store, &month_iter, COLUMN_LOG_DAY, utf8(str),
                            COLUMN_LOG_FILENAME, sort_key, COLUMN_LOG_TYPE,
                            LOG_TYPE_TOPLEVEL, -1);
         g_free(sort_key);
      ...
      gtk_tree_store_set(store, &date_iter, COLUMN_LOG_DAY, utf8(str),
                         COLUMN_LOG_FILENAME, logfile, COLUMN_LOG_TYPE,
                         LOG_TYPE_FILE, -1);
      g_free(date);
   }
   g_dir_close(dir);
      ...
      while (getline(in, s)) {
         ...
         if (str != NULL) {
            ...
            if (me->findStr) {
               ...
               if (off) {
                  gtk_text_buffer_get_iter_at_line_index(buffer, &start, line,
                                                         off - str);
                                ...
                                gpointer      data)
{
   ...
   gtk_tree_model_get(model, iter, COLUMN_LOG_FILENAME, &file, COLUMN_LOG_TYPE,
                      &type, -1);
   ...
   : RGGtkBuilderWindow(parent, "logview"), findStr(NULL)
{
   ...
   // find button
   g_signal_connect(gtk_builder_get_object(_builder, "button_find"), "clicked",
   ...
   g_signal_connect(G_OBJECT(select), "changed",
                    G_CALLBACK(cbTreeSelectionChanged), this);
   _textView = GTK_WIDGET(gtk_builder_get_object(_builder, "textview_log"));
   assert(_textView);

   g_signal_connect(gtk_builder_get_object(_builder, "entry_find"), "activate",
                    G_CALLBACK(cbButtonFind), this);

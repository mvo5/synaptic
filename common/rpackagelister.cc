...
bool RPackageLister::setSubView(string newSubView)
{
   if (_config->FindB("Debug::Synaptic::View", false))
      ioprintf(clog,
               "RPackageLister::setSubView(): newSubView '%s'\n",
...
void RPackageLister::notifyPostChange(RPackage *pkg)
{
   if (_config->FindB("Debug::Synaptic::View", false))
      ioprintf(clog,
               "RPackageLister::notifyPostChange(): '%s'\n",
                                  ...
                                  listSortMode mode)
{
   ...
   switch (mode) {
      ...
      case LIST_SORT_SIZE_ASC:
         stable_sort(
            packages.begin(), packages.end(), sortFunc<instSizeSortFunc>(true));
         break;
      case LIST_SORT_SIZE_DES:
         stable_sort(packages.begin(),
                     packages.end(),
                     sortFunc<instSizeSortFunc>(false));
         break;
      case LIST_SORT_DLSIZE_ASC:
         stable_sort(
            packages.begin(), packages.end(), sortFunc<dlSizeSortFunc>(true));
         break;
      case LIST_SORT_DLSIZE_DES:
         stable_sort(
            packages.begin(), packages.end(), sortFunc<dlSizeSortFunc>(false));
         break;
      ...
      case LIST_SORT_SECTION_ASC:
         stable_sort(
            packages.begin(), packages.end(), sortFunc<sectionSortFunc>(true));
         break;
      case LIST_SORT_SECTION_DES:
         stable_sort(
            packages.begin(), packages.end(), sortFunc<sectionSortFunc>(false));
         break;
      case LIST_SORT_STATUS_ASC:
         stable_sort(
            packages.begin(), packages.end(), sortFunc<statusSortFunc>(true));
         break;
      case LIST_SORT_STATUS_DES:
         stable_sort(
            packages.begin(), packages.end(), sortFunc<statusSortFunc>(false));
         break;
      case LIST_SORT_SUPPORTED_ASC:
         stable_partition(packages.begin(),
                          packages.end(),
      ...
      case LIST_SORT_VERSION_ASC:
         stable_sort(
            packages.begin(), packages.end(), sortFunc<versionSortFunc>(true));
         break;
      case LIST_SORT_VERSION_DES:
         stable_sort(
            packages.begin(), packages.end(), sortFunc<versionSortFunc>(false));
         break;
...
int RPackageLister::findNextPackage()
{
   ...
   for (unsigned i = _searchData.last + 1; i < _viewPackages.size(); i++) {
      if (_searchData.isRegex) {
         if (regexec(
                &_searchData.regex, _viewPackages[i]->name(), 0, NULL, 0) ==
             0) {
            _searchData.last = i;
            return i;
         }
...
bool RPackageLister::xapianSearch(string unsplitSearchString)
{
   ...
   try {
      ...
      Xapian::Query query =
         parser.parse_query(unsplitSearchString,

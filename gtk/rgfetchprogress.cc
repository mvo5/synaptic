...
enum { DLDone = -1, DLQueued = -2, DLFailed = -3, DLHit = -4 };
   ...
   : RGGtkBuilderWindow(win, "fetch"), _cursorDirty(false)
{
   ...
   _tableListStore =
      gtk_list_store_new(N_FETCH_COLUMNS, G_TYPE_INT, G_TYPE_STRING,
   ...
   column   = gtk_tree_view_column_new_with_attributes(
      _("Status"), renderer, "value", FETCH_PROGRESS_COLUMN, "text",
      FETCH_PROGRESS_TEXT_COLUMN, NULL);
   ...
   column   = gtk_tree_view_column_new_with_attributes(
      _("Size"), renderer, "text", FETCH_SIZE_COLUMN, NULL);
   gtk_tree_view_append_column(GTK_TREE_VIEW(_table), column);

   /* descr */
   renderer = gtk_cell_renderer_text_new();
   column   = gtk_tree_view_column_new_with_attributes(
      _("Package"), renderer, "text", FETCH_DESCR_COLUMN, NULL);
   gtk_tree_view_append_column(GTK_TREE_VIEW(_table), column);

   /* url */
   renderer = gtk_cell_renderer_text_new();
   column = gtk_tree_view_column_new_with_attributes(_("URI"), renderer, "text",
                                                     FETCH_URL_COLUMN, NULL);
   gtk_tree_view_append_column(GTK_TREE_VIEW(_table), column);
   ...
   // emit a signal if the user changed the cursor
   GtkWidget *expander =
      GTK_WIDGET(gtk_builder_get_object(_builder, "expander"));
   assert(expander);
   g_signal_connect(expander, "notify::expanded", G_CALLBACK(expanderActivate),
                    this);
}
                                       ...
                                       gpointer    data)
{
   ...
   GtkWidget *win =
      GTK_WIDGET(gtk_builder_get_object(me->_builder, "window_fetch"));
...
bool RGFetchProgress::Pulse(pkgAcquire *Owner)
{
...
#else
      if (I->TotalSize > 0)
         updateStatus(*I->CurrentItem, long(double(I->CurrentSize * 100.0) /
                                            double(I->TotalSize)));
#endif
      else
   ...
   if (CurrentCPS != 0 && ETA != 0) {
      s =
         g_strdup_printf(_("Download rate: %s/s - %s remaining"),
                         SizeToStr(CurrentCPS).c_str(), TimeToStr(ETA).c_str());
...
void RGFetchProgress::refreshTable(int row, bool append)
{
   ...
   if (append == true) {
      ...
      if (!_cursorDirty) {
         ...
         gtk_tree_view_scroll_to_cell(GTK_TREE_VIEW(_table), path, NULL, TRUE,
                                      0.0, 0.0);
         gtk_tree_path_free(path);
      }
   } else {
      gtk_tree_model_iter_nth_child(GTK_TREE_MODEL(_tableListStore), &iter,
                                    NULL, row);
   }

   // set the data
   gtk_list_store_set(
      _tableListStore, &iter, FETCH_PROGRESS_COLUMN,
      getStatusPercent(_items[row].status), FETCH_PROGRESS_TEXT_COLUMN,
      getStatusStr(_items[row].status), FETCH_SIZE_COLUMN,
      _items[row].size.c_str(), FETCH_DESCR_COLUMN, _items[row].descr.c_str(),
      FETCH_URL_COLUMN, _items[row].uri.c_str(), -1);
   path = gtk_tree_model_get_path(GTK_TREE_MODEL(_tableListStore), &iter);
   if (!_cursorDirty)
      gtk_tree_view_scroll_to_cell(GTK_TREE_VIEW(_table), path, NULL, TRUE, 0.0,
                                   0.0);
   gtk_tree_path_free(path);
}

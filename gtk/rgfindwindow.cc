...
void RGFindWindow::doFind(GtkWindow *widget, void *data)
{
   ...
   GtkListStore *prevSearchStore =
      GTK_LIST_STORE(gtk_combo_box_get_model(GTK_COMBO_BOX(me->_comboFind)));
   ...
   : RGGtkBuilderWindow(win, "find"), _prevSearches(0)
{
   ...
   _comboFind =
      GTK_WIDGET(gtk_builder_get_object(_builder, "comboentry_search"));
   assert(_comboFind);
   g_signal_connect(G_OBJECT(_comboFind), "changed", G_CALLBACK(cbEntryChanged),
                    this);
   ...
   gtk_cell_layout_add_attribute(GTK_CELL_LAYOUT(_comboFind), crt, "text", 0);

   _comboSearchType =
      GTK_WIDGET(gtk_builder_get_object(_builder, "combo_lookin"));
   assert(_comboSearchType);
   comboStore =
      GTK_LIST_STORE(gtk_combo_box_get_model(GTK_COMBO_BOX(_comboSearchType)));
   ...
   gtk_cell_layout_add_attribute(GTK_CELL_LAYOUT(_comboSearchType), crt, "text",
                                 0);
   ...
   assert(_findB);
   g_signal_connect(G_OBJECT(_findB), "clicked", G_CALLBACK(doFind), this);
